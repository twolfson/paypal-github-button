...
window.addEventListener('load', function handleLoad () {
  ...
  // viewBox="0 0 30 30"
  var viewBoxArr = svgDoc.childNodes[0].getAttribute('viewBox').split(' ');
  var viewBoxWidth = parseInt(viewBoxArr[2], 10);
  var viewBoxHeight = parseInt(viewBoxArr[3], 10);
  ...
  retInfo = {
    ...
    logo: _.extend({},
      ...
      // Computed scaled bounds
      {
        ...
        // TODO: I thought we had it but I guess not =(...
        ratio: (function () {
          // Calculate how tall our viewBox is when scaled up in the HTML
          // i.e. Our SVG is 15x15 but 30x30 in HTML
          //   thus our viewBox for it is 20x20 -> 40x40
          var logoRatio = logoBounds.height / svgHeight;
          var newViewBoxHeight = viewBoxHeight * logoRatio;

          // Calculate how much we need to scale our view box to match the end view box
          var endViewBoxHeight = buttonBounds.height;
          var viewBoxRatio = endViewBoxHeight / newViewBoxHeight;
